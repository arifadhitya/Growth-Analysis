...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    ...
    <h1 class="h2">Ubah Data Transaksi {{ $pembelian->kodetransaksi }}</h1>
    ...
    <!-- Modal Input -->
        ...
        @csrf
            <div class="mb-3 {{ $errors->has('kodetransaksi') ? ' has-error' : '' }}">
                ...
                <input type="text" class="form-control @error('kodetransaksi') is-invalid @enderror" id="kodetransaksi" name="kodetransaksi" required value="{{ old('kodetransaksi', $pembelian->kodetransaksi) }}">
            @error('kodetransaksi')
            ...
            <div class="mb-3" {{ $errors->has('operator') ? ' has-error' : '' }}>
                ...
                <input type="text" class="form-control @error('operator') is-invalid @enderror" id="operator" name="operator" value="{{ old('operator', $pembelian->operator) }}">
                @error('operator')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
            ...
            <div class="mb-3" {{ $errors->has('totalbayar') ? ' has-error' : '' }}>
                ...
                <input type="text" class="form-control @error('totalbayar') is-invalid @enderror" id="totalbayar" name="totalbayar" value="{{ old('totalbayar', $pembelian->kodetransaksi) }}">
                @error('totalbayar')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
            ...
            <div class="mb-3" {{ $errors->has('tanggaltransaksi') ? ' has-error' : '' }}>
                ...
                <input type="text" class="form-control @error('tanggaltransaksi') is-invalid @enderror datepicker" id="tanggaltransaksi" name="tanggaltransaksi" value="{{ old('tanggaltransaksi', $pembelian->kodetransaksi) }}">

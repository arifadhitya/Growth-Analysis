...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    ...
    <!-- Modal Input -->
        <!-- Formnya -->
        <form action="/staf" method="POST" accept-charset="UTF-8" enctype="multipart/form-data">
        @csrf
            <div class="mb-3 {{ $errors->has('kodepegawai') ? ' has-error' : '' }}">
                <label for="kodepegawai" class="form-label">Nomor Induk</label>
                <input type="text" class="form-control @error('kodepegawai') is-invalid @enderror" id="kodepegawai" name="kodepegawai" required value="{{ old('kodepegawai') }}">
            @error('kodepegawai')
            ...
            <div class="mb-3" {{ $errors->has('namapegawai') ? ' has-error' : '' }}>
                ...
                <input type="text" class="form-control @error('namapegawai') is-invalid @enderror" id="namapegawai" name="namapegawai" required value="{{ old('namapegawai') }}">
                @error('namapegawai')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
